...
static const struct hdmi_video_timing hdmi_mode[] = {
	...
	},
	{
		...
		.vic = HDMI_1920X358P_60HZ,
		.vic_2nd = HDMI_1920X358P_60HZ,
		.pixelrepeat = 1,
		.interface = OUT_P888,
	},
	{
		...
		.vic = HDMI_1920X538P_60HZ,
		.vic_2nd = HDMI_1920X538P_60HZ,
		.pixelrepeat = 1,
		.interface = OUT_P888,
	}
};
	...
	list_for_each(pos, head) {
		...
		for (i = 0; i < ARRAY_SIZE(hdmi_mode); i++) {
			...
			if (vic == hdmi_mode[i].vic ||
			    ...
			    /* For some TV, e.g. Haier LS49AL88A92. There is no YUV420 descriptor
			    * but contain 4k 60/50 RGB444 mode, e.g. 3840x2160p-60.
			    * So we think it support 4K 60/50 YCbCr420 mode.
			    */
