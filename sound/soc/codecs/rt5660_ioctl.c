/*
 * rt5660_ioctl.h  --  RT5660 ALSA SoC audio driver IO control
 *
...
#define RT5660_HWEQ_LEN ARRAY_SIZE(hweq_param)
	...
	struct snd_soc_codec *codec, int channel, int mode)
{
	...
	if (codec == NULL ||  mode >= RT5660_HWEQ_LEN)
		return -EINVAL;
	...
	case EQ_CH_DACR:
		...
		break;
	//case EQ_CH_ADC:
	//	reg = RT5660_ADC_EQ_CTRL2;
	//	mask = 0x01bf;
	//	upd_reg = RT5660_ADC_EQ_CTRL1;
	//	break;
	default:
	...
	snd_soc_update_bits(codec, upd_reg,
		RT5660_EQ_UPD, RT5660_EQ_UPD);
	snd_soc_update_bits(codec, upd_reg, RT5660_EQ_UPD, 0);

	return 0;
}

int rt5660_ioctl_common(struct snd_hwdep *hw, struct file *file,
			unsigned int cmd, unsigned long arg)
{
	...
	case RT_SET_CODEC_HWEQ_IOCTL:
		if (eq_mode == *buf)
			break;
		eq_mode[*buf] = *(buf + 1);
		rt5660_update_eqmode(codec, eq_mode[*buf], *buf);
		break;

	case RT_GET_CODEC_ID:
		*buf = snd_soc_read(codec, RT5660_VENDOR_ID2);
...
EXPORT_SYMBOL_GPL(rt5660_ioctl_common);
