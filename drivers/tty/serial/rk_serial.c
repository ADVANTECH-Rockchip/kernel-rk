...
{
	struct uart_rk_port *up = container_of(timer, struct uart_rk_port, tx_timer);
	unsigned int set = 0;
	unsigned int status;
	...
	if ((status & UART_LSR_TEMT) == UART_LSR_TEMT){
		set = 1;
		goto ret;
	}
...
cont:
	...
	if (up->wait_count)
		hrtimer_start(&up->tx_timer, ktime_set(0, 2000000), HRTIMER_MODE_REL);

ret:
	if(set)
		gpio_direction_output(up->rs485_gpio,!up->rs485_gpio_active);
...
static void serial_rk_handle_port(struct uart_rk_port *up)
{
	...
	{   /* dma mode disable */
		...
		/*
		 * when FIFO and THRE mode both are enabled,and
		 * FIFO TX empty trigger is set to larger than 1,
		 * ,we need to add ((up->iir & 0x0f) == 0x02) to transmit_chars,
		 * because when entering interrupt,the FIFO and THR might not be 1.
		 */
		if ((up->iir & 0x0f) == 0x02)
			transmit_chars(up);
	}
		      ...
		      struct ktermios *old)
{
	...
	*/
	...
	/* to avoid uart busy when set baud rate */
...
#ifdef CONFIG_ARCH_ADVANTECH
static int of_adv_parse_dt(struct device dev, struct uart_rk_port *up)
{
	...
	if (gpio_is_valid(uart_mode_sel_gpio))
	{
		...
		if(gpio_get_value(uart_mode_sel_gpio) == 0){
			...
			up->rs485_gpio = of_get_named_gpio_flags(np, "rs485-dir-gpio", 0, &flags);
			up->rs485_gpio_active = (flags == GPIO_ACTIVE_HIGH)? 1:0;
			if (gpio_is_valid(up->rs485_gpio))
			{
				gpio_request(up->rs485_gpio,"RS485 Select");
				gpio_direction_output(up->rs485_gpio,!up->rs485_gpio_active);
				up->rs485.flags = SER_RS485_ENABLED | SER_RS485_RTS_ON_SEND;
			}
		}else{
...
static int serial_rk_probe(struct platform_device *pdev)
{
	...
	pdev->id = rks.id;
#ifdef CONFIG_ARCH_ADVANTECH
	of_adv_parse_dt(pdev->dev,up);
	hrtimer_init(&up->tx_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	up->tx_timer.function = serial_rk_report_dma_tx;
#endif
#endif
